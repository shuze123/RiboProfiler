...
bowtie2_align <- function(index = NULL,fq_file1 = NULL,fq_file2 = NULL,
 ...
 # 构建临时参数
  ...
  # 运行比对
  map <- system2(bowtie_path, final_params, stdout = TRUE, stderr = TRUE)
...
batch_sam2bam <- function(sam_file = NULL, bam_file = NULL, threads = 1) {
  lapply(seq_along(sam_file), function(x) {
    ...
    system2("/usr/local/bin/samtools/samtools", args = c("view", paste0("-@ ", threads), "-Sb", sam_file[x], "-o", bam_file_path))
    system2("/usr/local/bin/samtools/samtools", args = c("sort", paste0("-@ ", threads), bam_file_path, "-o", sorted_bam_file_path))
