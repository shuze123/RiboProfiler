...
#' (default is 35).
#'
#' @return No explicit return value. Output files are written to "2_density_data" directory.
#'
...
#' The output file(s) will be saved in the "2_density_data" directory.
#'
#' @return NULL
#'
#' @examples
...
pre_rna_coverage_data <- function(sam_file = NULL,
  ...
  if(!is.null(bam_file)){
    lapply(seq_along(bam_file), function(x){
      ...
      # output
      data.table::fwrite(depth,file = outFile_tmp,sep = "\t",col.names = FALSE,nThread = 8)
...
#' the XAM package in Julia. It writes the output files to the "2_density_data"
#' directory.
#'
...
load_qc_data <- function(sample_name = NULL,
  ...
  plyr::ldply(1:length(file),function(x){
    tmp <- data.table::fread(paste('1_QC_data/',file[x],sep = ''))
...
load_track_data <- function(mapping_type = c("genome","transcriptome"),
  ...
  if(mapping_type == "genome"){
    plyr::ldply(1:length(ribo_file),function(x){
      # ribo denisty
      # ribo_tmp <- data.table::fread(paste('2_density_data/',ribo_file[x],sep = ''),sep = "\t")
      ribo_tmp <- vroom::vroom(paste('2_density_data/',ribo_file[x],sep = ''),
      ...
      if(!is.null(rna_file)){
        # rna coverage
        # rna_tmp <- data.table::fread(paste('2_density_data/',rna_file[x],sep = ''),sep = "\t")

        rna_tmp <- vroom::vroom(paste('2_density_data/',rna_file[x],sep = ''),
                                delim = "\t",show_col_types = FALSE,
                                col_names = c('gene_name',"trans_id",'transpos','density'))
        # add colnames
    ...
    plyr::ldply(1:length(ribo_file),function(x){
      # ribo denisty
      # ribo_tmp <- data.table::fread(paste('2_density_data/',ribo_file[x],sep = ''),
      #                               sep = "\t")[,c(1,2,4)]
      ribo_tmp <- vroom::vroom(paste('2_density_data/',ribo_file[x],sep = ''),
                               delim = "\t",show_col_types = FALSE,
      ...
      if(!is.null(rna_file)){
        # rna coverage
        # rna_tmp <- data.table::fread(paste('2_density_data/',rna_file[x],sep = ''),
        #                              sep = "\t")[,c(1,2,4)]
        rna_tmp <- vroom::vroom(paste('2_density_data/',rna_file[x],sep = ''),
                                delim = "\t",show_col_types = FALSE,
                                col_names = c('id',"transpos",'none','density'),
...
#' # files called 'file1.bam' and 'file2.bam' in '2_density_data/' directory. The following code
#' # will calculate the metagene data and save the results in '3_metagene_data/' directory with
...
load_metagene_data <- function(sample_name = NULL,
                               group_name = NULL){
  # load data
  file <- list.files('3_metagene_data/','.txt')
  ...
  plyr::ldply(1:length(file),function(x){
    tmp <- data.table::fread(paste('3_metagene_data/',file[x],sep = ''))
...
load_expression_data <- function(sample_name = NULL){
  # load data
  file <- list.files('4_expression_data/','.txt')
  ...
  lapply(1:length(file),function(x){
    tmp <- data.table::fread(paste('4_expression_data/',file[x],sep = ''))[,-3]
    # add sample
  ...
  # extract tpm data
  lapply(1:length(file),function(x){
    tmp <- data.table::fread(paste('4_expression_data/',file[x],sep = ''))[,-2]
    # add sample
    sample <- sample_name[x]
    colnames(tmp) <- c("gene_name",sample)
